<?php
	...
	if(isset($_GET['x']) && isset($_GET['y']) && isset($_GET['w']) && isset($_GET['m'])){
		...
		echo "<script>location.href='admin_minggu';</script>";
	}
...
<body>
	...
	<main id="main">
		...
		<section class="breadcrumbs">
			...
			<li><a href="admin_minggu?m=<?php echo $minggu; ?>">Minggu <?php echo $minggu; ?></a></li>
			...
			<div class="container">
				<form action='' method='POST'>
						...
						<div class="form-group">
							<div class="">
								<div class="form-floating">
									<select class="form-control mb-1" name="inputID" id="inputID" required onchange="getID(this)">
										<option value="">Sila pilih</option>
										<?php
											$sql = "SELECT id_pensyarah, nama FROM pensyarah ORDER BY nama ASC;";
											$result = mysqli_query($db, $sql);
											while($row = mysqli_fetch_array($result)){
												if(isset($_GET['id'])){
													if($row['id_pensyarah'] == $_GET['id']){
														echo "
															<option selected
																value='".$row['id_pensyarah']."'>".$row['nama'].", ".$row['id_pensyarah']."
															</option>
														";
													}else{
														echo "
						...
						<div class="form-group row mt-3">
							<div class="">
								<div class="form-floating">
									...
									<?php
									if(isset($_GET['id'])){
										$sql = "SELECT kursus FROM pensyarah WHERE id_pensyarah = '".$_GET['id']."';";
										$result = mysqli_query($db, $sql);
										...
										if($row != 0){
											...
											echo '
											<select class="form-control mb-1" name="inputKursus" id="inputKursus" name="kursus" disabled required>
												<option value="Tiada kursus">Tiada kursus</option>
											</select>
											';
										}
									}else{
										echo '
											<select class="form-control mb-1" name="inputKursus" id="inputKursus" name="kursus" disabled required>
												<option value="">Sila pilih ID Pensyarah</option>
											</select>
										';
									}
									?>
						...
						<div class="form-group mt-3">
							<div class="">
								<div class="form-floating">
									<input type="text" class="form-control" name="inputTujuan" id="inputTujuan" placeholder="Tujuan" required value="<?php
							...
							if(isset($_POST['submit'])){
								...
								if($count == 0){
									...
									if($result){
										...
										<script>
											location.href="admin_minggu?m='.$minggu.'";
										</script>
										';
									}else{
										$_POST['salah'] = true;
	...
	function getID(y){
		var x = y.value;
		if(x != ""){
			location.href="<?php echo 'admin_tempah?x='.$masa.'&y='.$hari.'&w='.$week.'&m='.$minggu.'&id='; ?>"+x;
		}else{
			location.href="<?php echo 'admin_tempah?x='.$masa.'&y='.$hari.'&w='.$week.'&m='.$minggu; ?>";
		}
	}
	</script>
</body>
