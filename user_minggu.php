...
<body>
    <?php
        if(isset($_GET['kod'])){
            ...
            showNotification("Kod Verifikasi bagi penempahan yang baru dibuat ialah '.$_GET['kod'].'");
            </script>
            ';
        }
    ?>
		...
		<section class="inner-page">
			<div class="container">
				...
				if(!isset($current_user)){
					...
					Sila <a href="user_login" class="alert-link">Log In</a> untuk membuat penempahan.
					</div>
					';
				}else{
					echo '
					...
					<tbody>
						...
						for($i = 0;$i < count($hari);$i++ ){
							...
							for ($col = 0; $col < 8; $col++) {
								...
								if($valid == "-"){
									if(isset($current_user)){
										echo "<td class='border p-0 align-middle'><a href='user_tempah?x=".($col+1)."&y=".$i."&w=".$thisweek."&m=".$minggu."'>
										<div class='w-100 h-100 pt-1 pb-1 m-auto'>&nbsp;</div>
										</a></td>";
									}else{
										echo "<td class='border' data-bs-toggle='tooltip' data-bs-placement='bottom' data-bs-html='true' title='Sila login untuk membuat tempahan'></td>";
									}
								}else{
									$row = explode('| ', $valid);
									if(isset($current_user)){
										if($row[1] == $current_user){
											echo "
											<td class='bg-warning align-middle ' data-bs-toggle='tooltip' data-bs-placement='bottom' data-bs-html='true' title='Kod: ".$row[3]."'>
											<a class='text-dark' role='button' aria-expanded='false' aria-controls='info-".$row[4]."' data-bs-toggle='collapse' href='#info-".$row[4]."'>You</a>
											";
										}else{
											echo "
											<td class='align-middle' data-bs-toggle='tooltip' data-bs-placement='bottom' data-bs-html='true' title='NoTel: ".$row[2]."'>
